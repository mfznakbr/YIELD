...
if __name__ == "__main__":
    ...
    test_path = sys.argv[3] if len(sys.argv) > 3 else os.path.join(os.path.dirname(os.path.abspath(__file__)), "testing.xlsx")

    print(f"Reading train data from: {file_path}")

    # Load data
    train_df = pd.read_excel(file_path)
    test_df = pd.read_excel(test_path)  # Pastikan file ini juga tersedia saat CI/CD
